...
const HIPPO = """
M148.802,244.876c2.737-36.735,16.107-69.079,40.099-97.061
...
const ELEPHANt =
    """M446.47,99.669c12.095,0,23.828,7.003,35.234,20.993c11.378,13.989,17.971,20.993,19.755,20.993
...
class _HomeState extends State<Home> with SingleTickerProviderStateMixin {
  ...
  SampledPathData sampledPathData;
  @override
  void initState() {
    super.initState();
    final matrix = Matrix4.identity()..scale(0.5, 1);
    Path path1 = parseSvgPathData(HIPPO)..transform(matrix.storage);
    Path path2 = parseSvgPathData(ELEPHANt)..transform(matrix.storage);
    sampledPathData = PathMorph.samplePaths(path2, path1);
    animationController = AnimationController(
      vsync: this,
      duration: Duration(seconds: 1),
    );
    PathMorph.generateAnimations(animationController, sampledPathData, func);
  }

  void func(int i, Offset offset) {
    setState(() {
      sampledPathData.shiftedPoints[i] = offset;
    });
  }
          ...
          children: <Widget>[
            ...
            FittedBox(
              ...
              child: SizedBox(
                ...
                child: CustomPaint(
                  painter: MyPainter(PathMorph.generatePath(sampledPathData)),
                ),
              ),
            ),
            Spacer(),
